...
import { deletePhoto } from "../../../../../service/galleryService";
...
const PhotoItem = ({ 
  ...
  const handleDeletePhoto = async (e) => {
    ...
    const result = await Swal.fire({
      ...
      html: `
        <div class="text-left">
          <img src="${photo.image_url}" alt="Preview" class="w-32 h-32 object-cover mx-auto rounded-lg mb-3" />
          <p><strong>Album:</strong> ${photo.albums?.title || 'Unknown'}</p>
          <p><strong>Upload:</strong> ${new Date(photo.created_at).toLocaleDateString('id-ID')}</p>
        </div>
      `,
      icon: "warning",
  ...
  return (
    <div className="relative group bg-white rounded-lg shadow-sm hover:shadow-lg transition-all duration-200 overflow-hidden">
      ...
      <button
        ...
        disabled={isDeleting}
        className="absolute top-2 right-2 z-10 bg-red-500 hover:bg-red-600 text-white p-1.5 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
        title="Hapus Foto"
      >
        {isDeleting ? (
